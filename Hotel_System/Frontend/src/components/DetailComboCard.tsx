...
  if (!combo) return null;
  ...
  // LOGIC GIỮ NGUYÊN TỪ COMPONENT CỦA BẠN
  ...
  const giaCombo = typeof combo.comboPrice === 'number' 
    ? combo.comboPrice
    : (loai === 'percent'
      ? Math.round(originalPrice * (1 - (giaTri / 100)))
      : Math.round(Math.max(0, originalPrice - giaTri)));

  const tietKiem = Math.max(0, originalPrice - giaCombo);
  const discountPercent = originalPrice ? Math.round((tietKiem / originalPrice) * 100) : 0;
...
              <span style={{ color: 'green', fontWeight: FONT_WEIGHT_BOLD }}>{currency.format(tietKiem)} ({discountPercent}%)</span>
            </div>
            
