 ...
 * Update an amenity
 */
export const updateAmenity = async (id: string, tenTienNghi: string): Promise<void> => {
  if (!tenTienNghi || tenTienNghi.trim().length === 0) {
    throw new Error("Amenity name is required");
  }
  if (tenTienNghi.length > 100) {
 ...
 */
export const assignAmenityToRoom = async (roomId: string, amenityId: string): Promise<void> => {
  if (!roomId || !amenityId) {
  ...
  try {
    ...
    console.error(`Failed to assign amenity ${amenityId} to room ${roomId}:`, error);
    throw error;
  }
};
 ...
 */
export const removeAmenityFromRoom = async (roomId: string, amenityId: string): Promise<void> => {
  if (!roomId || !amenityId) {
    throw new Error("Room ID and Amenity ID are required");
  }

  try {
    const response = await fetch(`${API_BASE}/TienNghiPhong/${roomId}/${amenityId}`, {
      method: "DELETE"
    });
    ...
    console.error(`Failed to remove amenity ${amenityId} from room ${roomId}:`, error);
    throw error;
  }
};
