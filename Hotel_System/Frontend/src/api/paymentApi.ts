...
const API_BASE = ""; // keep empty to use relative paths via Vite proxy
...
async function payEWallet(path: string, idHoaDon: string, amount: number, phone?: string | null, note?: string) {
	const res = await fetch(`${API_BASE}${path}`, {
		method: "POST",
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({ IdHoaDon: idHoaDon, Amount: amount, EWalletPhone: phone, Note: note }),
	});
	const data = await handleJson<any>(res);
	return normalizePaymentResponse(data);
}
