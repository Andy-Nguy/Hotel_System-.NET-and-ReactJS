...
export const AmenticsRoomSection = () => {
  ...
  const [selectedAmenitiesMap, setSelectedAmenitiesMap] = useState<Record<string, boolean>>({});
            ...
            {loading ? (
              ...
              rooms.map(room => (
                <button
                  ...
                  style={{
                    ...styles.roomItem,
                    ...(selectedRoom?.idphong === room.idphong ? styles.roomItemActive : {})
                  }}
          ...
          {selectedRoom ? (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <h2 style={styles.panelTitle}>Tiện nghi của {selectedRoom.tenPhong}</h2>
                <div style={{ color: '#6b7280', fontSize: 13 }}>{Object.values(selectedAmenitiesMap).filter(Boolean).length} đã chọn</div>
              </div>

              <div style={styles.amenitiesSearchWrap}>
                <input
                  placeholder="Tìm tiện nghi trong phòng..."
                  style={styles.searchInputSmall}
                  onChange={(e) => {
                    // lightweight local filter - keep original amenities
                    const q = e.target.value.toLowerCase();
                    if (!q) setAmenities((a) => a.slice());
                    else setAmenities((a) => a.filter(am => (am.tenTienNghi || '').toLowerCase().includes(q)));
                  }}
                />
              </div>
              ...
              {loading ? (
                ...
                <div style={styles.amenitiesGrid}>
                  {amenities.map(amenity => {
                    ...
                    return (
                      <label key={amenity.idtienNghi} style={{ ...styles.amenityPill, ...(checked ? styles.amenityPillActive : {}) }}>
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={() => handleAmenityToggle(amenity.idtienNghi)}
                          style={{ display: 'none' }}
                          disabled={saving}
                        />
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={{ fontWeight: 600 }}>{amenity.tenTienNghi}</div>
                          <div style={styles.amenityCountPill}>{amenity.roomCount || 0}</div>
                        </div>
                      </label>
                    );
                  })}
