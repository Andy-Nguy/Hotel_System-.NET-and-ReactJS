		...
		<tr style={{ borderBottom: '1px solid #f3f4f6' }}>
			...
			<td style={{ padding: 12, verticalAlign: 'middle', color: '#6b7280' }}>{service.tienDichVu != null ? new Intl.NumberFormat('vi-VN').format(service.tienDichVu) : ''}</td>
					<td style={{ padding: 12, verticalAlign: 'middle' }}>
						<button onClick={() => onDetails && onDetails(service)} style={{ marginRight: 8, padding: '6px 10px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff' }}>Chi tiết</button>
						<button onClick={() => onEditModal ? onEditModal(service) : onEdit && onEdit(service)} style={{ marginRight: 8, padding: '6px 10px', borderRadius: 8, background: 'linear-gradient(135deg,#1e40af,#3b82f6)', color: '#fff', border: 'none', fontWeight: 700 }}>Sửa</button>
						<button onClick={() => onDelete && onDelete(service.iddichVu)} style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid #f3f4f6' }}>Xóa</button>
					</td>
		</tr>
	);
};
