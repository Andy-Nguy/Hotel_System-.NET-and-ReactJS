...
import { Card, Form, Input, Button, Select, Upload, Spin, Modal, message, Space, Alert } from 'antd';
...
interface BlogFormData {
  ...
  images: string[];
  externalLink: string;
  status: 'DRAFT' | 'PENDING' | 'APPROVED' | 'REJECTED' | 'PUBLISHED' | 'HIDDEN' | 'ARCHIVED' | 'DELETED';
  slug?: string;
...
const BlogEdit: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        title: (formData.title?.trim() || '') !== (originalData.title?.trim() || '') ? formData.title?.trim() : originalData.title,
        category: formData.category !== originalData.category ? formData.category : originalData.category,
        type: formData.type !== originalData.type ? formData.type : originalData.type,
        image: formData.image !== originalData.image ? formData.image : originalData.image,
        date: (formData.date?.trim() || '') !== (originalData.date?.trim() || '') ? 
          (formData.date?.trim() || new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })) : 
          originalData.date,
        excerpt: (formData.excerpt?.trim() || '') !== (originalData.excerpt?.trim() || '') ? formData.excerpt?.trim() : originalData.excerpt,
        author: (formData.author?.trim() || '') !== (originalData.author?.trim() || '') ? formData.author?.trim() : originalData.author,
        // Backend expects tags as a comma-separated string (e.g. "tag1, tag2").
        tags: JSON.stringify(currentTags) !== JSON.stringify(originalTags) ? currentTags.join(', ') : (Array.isArray(originalTags) ? originalTags.join(', ') : (originalTags || '')),
        content: (formData.content?.trim() || '') !== (originalData.content?.trim() || '') ? formData.content?.trim() : originalData.content,
        images: JSON.stringify(formData.images || []) !== JSON.stringify(originalData.images || []) ? formData.images : originalData.images,
        externalLink: (formData.externalLink?.trim() || '') !== (originalData.externalLink?.trim() || '') ? formData.externalLink?.trim() : originalData.externalLink,
        ...
        <main style={{ padding: '0px 60px' }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 8px 24px rgba(2,6,23,0.06)' }}>
              ...
              <div style={{ fontSize: 12, color: '#666' }}>
                Trạng thái: <strong>{formData ? getStatusLabel(formData.status) : 'Đang tải...'}</strong>
              </div>
            </div>

            {/* Show warning if blog is in rejection state */}
            {formData && formData.status === 'REJECTED' && formData.rejectionReason && (
              <Alert
              ...
              <Card style={{ marginBottom: 20 }}>
                <Form layout="vertical" form={form}>
                  ...
                  <Form.Item 
                    label="Tiêu đề *" 
                    required
                    help={formData && formData.title && formData.title.length < 3 ? "Tiêu đề phải có ít nhất 3 ký tự" : ""}
                    validateStatus={formData && formData.title && formData.title.length < 3 ? "warning" : ""}
                  >
                    <Input
                  ...
                  <Form.Item label="Ảnh Cover *" required>
                    ...
                    >
                      <Button icon={<UploadOutlined />}>Upload Ảnh Cover Mới</Button>
                    </Upload>
                  </Form.Item>
                  ...
                  <Form.Item label="Trạng thái (Chỉ xem)">
                    ...
                    <div style={{ fontSize: 12, color: '#888', marginTop: 4 }}>
                      Sử dụng các nút hành động trong trang quản lý để thay đổi trạng thái bài viết
                    </div>
                  </Form.Item>
