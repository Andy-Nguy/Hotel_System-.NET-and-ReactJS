...
interface Props {
  ...
  // Indicates this invoice/booking was extended (gia hạn) – từ parent
  isExtended?: boolean;
}
...
const InvoiceModal: React.FC<Props> = ({
  ...
  const extractNotes = (data: any): string[] => {
    ...
    if (hdon?.GhiChu || hdon?.ghiChu)
      notes.push(String(hdon.GhiChu ?? hdon.ghiChu));

    const inv0 =
      Array.isArray(data?.invoices) && data.invoices.length > 0
        ? data.invoices[0]
        : null;
    if (inv0?.GhiChu || inv0?.ghiChu)
      notes.push(String(inv0.GhiChu ?? inv0.ghiChu));
  ...
  // Ưu tiên cờ isExtended từ parent, nếu không thì fallback theo ghi chú
  const hasExtendNote = isExtended || notes.some((n) => n.includes('gia han'));
  ...
  const pctCandidates = [
    moneyAny.extendPercent,
    moneyAny.extend_pct,
    moneyAny.extendRate,
    moneyAny.extendRatePercent,
  ];
  ...
  const lateFeeSrv = Number(money.lateFee ?? 0); // phí trả phòng muộn (nếu có)

  const roomTotal = !isNaN(roomTotalSrv) ? roomTotalSrv : computedRoomTotal;
  const serviceTotal = !isNaN(serviceTotalSrv)
    ? serviceTotalSrv
  ...
  // Tổng tiền tốt nhất từ hoá đơn nếu money.tongTien chưa kịp cập nhật
  ...
  const inv0 =
    Array.isArray(displayData?.invoices) && displayData.invoices.length > 0
      ? displayData.invoices[0]
      : null;
  ...
  // Ưu tiên chọn tổng lớn nhất & hợp lệ giữa money.tongTien, HoaDon.TongTien, invoices[0].TongTien
  const candidates = [totalSrv, hoaDonTotal, inv0Total].filter(
    (v) => !isNaN(v) && v > 0
  );
  const bestTotal =
    candidates.length > 0 ? Math.max(...candidates) : Math.round(subTotal + vat);
  ...
  return (
    <Modal
      title={
        invoiceData
          ? `Hóa đơn - ${getInvoiceId(displayData) ?? ''}`
          : 'Hóa đơn'
      }
      open={visible}
      onCancel={onClose}
        ...
        <div>
          ...
          >
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              {/* Hiển thị badge gia hạn nếu có ghi chú hoặc phí gia hạn hoặc prop isExtended */}
              {(hasExtendNote || extendFee > 0) && (
                <div style={{ marginTop: 8 }}>
                  ...
                  {(extendDurationLabel || extendPercent !== null) && (
                    ...
                    >
                      {extendDurationLabel && (
                        <span>Thời gian: {extendDurationLabel}</span>
                      )}
                      {extendDurationLabel &&
                        extendPercent !== null && <span> • </span>}
                      {extendPercent !== null && (
                        <span>Tỷ lệ: {extendPercent}%</span>
                      )}
                    </div>
                  )}
                </div>
