...
const RoomTypeSection: React.FC = () => {
  ...
  const openRooms = async (t: RoomType) => {
    ...
    try {
      ...
      setRoomsForType(
        all.filter((r) => String(r.idloaiPhong) === String(t.idLoaiPhong))
      );
      setRoomsAll(all);
    } catch (e) {
      console.error(e);
  ...
  const handleAddRoomSave = async (values: {
    tenPhong: string;
    soPhong?: string;
    ...
    try {
      ...
      setRoomsForType(
        all.filter(
          (r) => String(r.idloaiPhong) === String(activeType.idLoaiPhong)
        )
      );
      setRoomsAll(all);
  ...
  return (
    <div>
      ...
      >
        {types.map((t) => {
          const count = roomsAll.filter(
            (r) => String(r.idloaiPhong) === String(t.idLoaiPhong)
          ).length;
          return (
            <div
              key={t.idLoaiPhong}
              ...
              >
                <img
                  ...
                  onError={(e) => {
                    ...
                    if (c && !c.querySelector(".placeholder")) {
                      const ph = document.createElement("div");
                      ph.className = "placeholder";
                      ph.style.cssText =
                        "position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:40px;color:#9ca3af";
                      ph.innerHTML = "🏨";
                      c.appendChild(ph);
                    }
                  }}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        <div style={{ fontWeight: 700 }}>
                          {r.tenPhong} {r.soPhong ? `(${r.soPhong})` : ""}{" "}
                          {r.trangThai === "Trống"
                            ? "💚"
    ...
    >
      ...
      >
        <h3 style={{ marginTop: 0 }}>
          {initial ? "Sửa loại phòng" : "Thêm loại phòng"}
        </h3>
        ...
        >
          ...
          <button
            onClick={() =>
              onSave({ tenLoaiPhong: ten, moTa, urlAnhLoaiPhong: url })
            }
            style={{
              padding: "8px 12px",
              borderRadius: 8,
          ...
          <button
            onClick={() =>
              onSave({
                tenPhong: ten,
                soPhong: so,
                giaCoBanMotDem: typeof gia === "number" ? gia : undefined,
                urlAnhPhong: url,
                moTa,
                trangThai,
              })
