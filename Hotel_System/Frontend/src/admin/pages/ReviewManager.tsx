...
const ReviewManager: React.FC = () => {
  ...
  const loadReviews = async () => {
    ...
    try {
      ...
      message.info('Tính năng này sẽ được kích hoạt khi backend thêm endpoint /api/Review');
    } catch (e: any) {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'IsAnonym',
      key: 'IsAnonym',
      render: (isAnonym: number) => isAnonym ? <Tag color="blue">Ẩn danh</Tag> : <Tag>Công khai</Tag>,
    },
    {
      title: 'Trạng thái',
      dataIndex: 'IsApproved',
      key: 'IsApproved',
      render: (isApproved: number) => isApproved ? <Tag color="green">Đã duyệt</Tag> : <Tag color="orange">Chờ duyệt</Tag>,
    },
    {
      title: 'Ngày tạo',
      dataIndex: 'CreatedAt',
    ...
    },
    {
      ...
      render: (_: any, record: Review) => (
        <Space size="small">
          <Button type="text" size="small" icon={<EyeOutlined />} onClick={() => { setSelectedReview(record); setDetailModalVisible(true); }}>Xem</Button>
          <Button type="text" size="small" icon={<CheckOutlined />} onClick={() => approveReview(record.IDDanhGia)}>Duyệt</Button>
          <Button type="text" size="small" danger icon={<DeleteOutlined />} onClick={() => deleteReview(record.IDDanhGia)}>Xóa</Button>
        </Space>
      ),
    },
  ];
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
                <Card>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 28, fontWeight: 600, color: '#667eea' }}>{stats.totalReviews || 0}</div>
                    <div style={{ color: '#666', marginTop: 4 }}>Tổng đánh giá</div>
                  </div>
                </Card>
                <Card>
                  <div style={{ textAlign: 'center' }}>
                    <Rate disabled value={stats.averageRating || 0} style={{ fontSize: 24 }} />
                    <div style={{ color: '#666', marginTop: 4 }}>Điểm trung bình: {stats.averageRating?.toFixed(1)}</div>
                  </div>
                </Card>
                <Card>
                  <div style={{ textAlign: 'center' }}>
                    <Badge count={pending} style={{ backgroundColor: '#faad14' }} offset={[-5, 5]}>
                      <div style={{ fontSize: 28, fontWeight: 600, color: '#faad14' }}>{pending}</div>
                    </Badge>
                ...
                <Card>
                  <div style={{ textAlign: 'center' }}>
                    <Badge count={approved} style={{ backgroundColor: '#52c41a' }} offset={[-5, 5]}>
                      <div style={{ fontSize: 28, fontWeight: 600, color: '#52c41a' }}>{approved}</div>
                    </Badge>
            ...
            >
              {selectedReview && (
                <div style={{ lineHeight: 1.8 }}>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    <strong>Ẩn danh:</strong> {selectedReview.IsAnonym ? 'Có' : 'Không'}
                  </div>
                  <div style={{ marginBottom: 12 }}>
                    <strong>Trạng thái:</strong> {selectedReview.IsApproved ? <Tag color="green">Đã duyệt</Tag> : <Tag color="orange">Chờ duyệt</Tag>}
                  </div>
                  <div>
