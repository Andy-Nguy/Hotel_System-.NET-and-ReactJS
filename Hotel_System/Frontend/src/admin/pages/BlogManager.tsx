...
import { Card, Input, Table, Space, Button, Tag, message, Badge, Select } from 'antd';
import { EditOutlined, DeleteOutlined, PlusOutlined, PoweroffOutlined, UndoOutlined } from '@ant-design/icons';
...
const adminBlogPosts: AdminBlogPost[] = blogPosts.map((post, index) => ({
  ...post,
  status: (index === 0 ? 'PUBLISHED' : index === 1 ? 'ARCHIVED' : 'DRAFT') as AdminPostStatus,
  order: index,
...
const BlogManager: React.FC = () => {
    ...
    const fetchPosts = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (!mounted) return;
          const mapped = (data as any[]).map((d, idx) => ({
            id: d.id,
            title: d.title,
            category: d.category,
          ...
          const hasAny = mapped.some(m => Object.keys(desiredOrderByTitle).includes(m.title));
          if (hasAny) {
            const withOrder = mapped.map((m) => ({ ...m, order: (typeof desiredOrderByTitle[m.title] === 'number') ? desiredOrderByTitle[m.title] : (typeof m.order === 'number' ? m.order : 999) }));
            // fill remaining orders sequentially for items without explicit mapping
            let next = 0;
            const used = new Set<number>(withOrder.filter(x => typeof x.order === 'number' && x.order < 999).map(x => x.order as number));
            withOrder.forEach((w) => {
              if (w.order === 999) {
  ...
  const handleApprove = async (id: number) => {
    ...
    setPosts(prev => prev.map(p => p.id === id ? { ...p, status: 'PUBLISHED' as AdminPostStatus } : p));
  };
  ...
  const handleReject = async (id: number, reason: string) => {
    try {
      ...
      if (res.ok) {
        await res.json();
        setPosts(prev => prev.map(p => p.id === id ? { ...p, status: 'DRAFT' as AdminPostStatus, rejectionReason: reason } : p));
        return;
      }
    } catch (e) {
  ...
  const handleHide = async (id: number) => {
    try {
      ...
      if (res.ok) {
        await res.json();
        setPosts(prev => prev.map(p => p.id === id ? { ...p, status: 'ARCHIVED' as AdminPostStatus } : p));
  ...
  const updatePostStatus = (id: number, newStatus: AdminPostStatus) => {
    (async () => {
      ...
      const payload = {
        ...
        tags: (Array.isArray(post.tags) ? post.tags.filter(t => t).join(', ') : (post.tags || '')),
        images: post.images || [],
        viewCount: (post as any).viewCount ?? (post as any).view_count ?? 0,
        rejectionReason: (post as any).rejectionReason ?? (post as any).rejection_reason ?? '',
        rejectedAt: (post as any).rejectedAt ?? (post as any).rejected_at ?? null,
        approvedAt: (post as any).approvedAt ?? (post as any).approved_at ?? null,
        auditLog: (post as any).auditLog ?? (post as any).audit_log ?? '',
        createdAt: (post as any).createdAt ?? (post as any).created_at ?? new Date().toISOString(),
        updatedAt: now,
        publishedAt: (post as any).publishedAt ?? (post as any).published_at ?? null,
        displayOrder: null,
        authorId: (post as any).authorId ?? (post as any).author_id?.toString() ?? '',
      };
      
      try {
        const res = await fetch(`/api/blog/${post.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        if (res.ok) {
          const updated = await res.json();
          setPosts(prev => prev.map(p => {
            if (p.id === id) return ({ ...(p as any), status: updated.status || newStatus, updated_at: updated.updatedAt || now, displayOrder: (updated.displayOrder ?? (newStatus !== 'PUBLISHED' ? null : p.displayOrder)) } as AdminBlogPost);
            // if we just moved this post away from PUBLISHED, clear its displayOrder locally
            if (newStatus !== 'PUBLISHED' && p.displayOrder) return ({ ...p, displayOrder: null } as AdminBlogPost);
            return p;
          }));
          try { window.dispatchEvent(new CustomEvent('blogs-updated')); } catch {}
          message.success(newStatus === 'PUBLISHED' ? 'Đã kích hoạt/Xuất bản' : newStatus === 'ARCHIVED' ? 'Đã ngưng hoạt động/Lưu trữ' : `Cập nhật trạng thái thành ${newStatus}`);
          return;
        }
        ...
        setPosts(prevPosts => prevPosts.map(post => post.id !== id ? post : ({ ...post, status: newStatus, updated_at: now } as AdminBlogPost)));
        message.warning(`Lỗi API, cập nhật cục bộ thành công trạng thái: ${newStatus}`);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const updateDisplayOrder = (id: number, order: number | null) => {
    ...
    if (post.status !== 'PUBLISHED') {
      message.warning('Chỉ có thể đặt thứ tự hiển thị cho bài viết đã xuất bản');
      return;
    }
    (async () => {
      setLoading(true);
      try {
        ...
        const res = await fetch(`/api/blog/${id}/thu-tu-hien-thi`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(bodyPayload) });
        if (res.ok) {
          const updated = await res.json();
          // Apply updated displayOrder and clear any conflicting local entries
          setPosts(prev => prev.map(p => {
            if (p.id === id) return ({ ...(p as any), displayOrder: updated.displayOrder } as AdminBlogPost);
            if (updated.displayOrder != null && p.id !== id && p.displayOrder === updated.displayOrder) return ({ ...p, displayOrder: null } as AdminBlogPost);
            return p;
          }));
          try { window.dispatchEvent(new CustomEvent('blogs-updated')); } catch {}
          message.success(order ? `Đã đặt thứ tự hiển thị: ${order}` : 'Đã xóa thứ tự hiển thị');
          return;
        }
        try {
          const err = await res.json();
        ...
        setPosts(prevPosts => prevPosts.map(p => {
          if (p.id === id) return ({ ...p, displayOrder: order } as AdminBlogPost);
          if (order != null && p.displayOrder === order) return ({ ...p, displayOrder: null } as AdminBlogPost);
          return p;
        }));
  ...
  const handleTogglePublishStatus = (id: number, currentStatus: AdminPostStatus) => {
    // Nếu đang là PUBLISHED (Đang hoạt động) -> Chuyển sang ARCHIVED (Ngưng hoạt động)
    const newStatus: AdminPostStatus = (currentStatus === 'PUBLISHED') ? 'ARCHIVED' : 'PUBLISHED';
              ...
              <Table
                ...
                columns={[
                  ...
                  { title: 'Trạng thái', dataIndex: 'status', key: 'status', width: 140, render: (s: AdminPostStatus) => {
                      const map: any = { PUBLISHED: 'green', DRAFT: 'orange', ARCHIVED: 'default' };
                      return <Tag color={map[s] || 'default'}>{s}</Tag>;
                    }
                  },
                  { title: 'Thứ tự hiển thị', dataIndex: 'displayOrder', key: 'displayOrder', width: 160, render: (d: number | null, r: AdminBlogPost) => {
                      if (r.status !== 'PUBLISHED') return <span style={{ color: '#ccc' }}>N/A</span>;
                      return (
                        <Select
                          value={d || undefined}
                          ...
                          style={{ width: 120 }}
                          allowClear
                          onChange={(value: number | undefined) => updateDisplayOrder(r.id, value || null)}
                          onClick={(e: React.MouseEvent) => e.stopPropagation()}
                        >
                          {[1,2,3,4,5].map(num => <Select.Option key={num} value={num}>{num}</Select.Option>)}
                        </Select>
                      );
                    }
                  },
                  { title: 'Ngày', dataIndex: 'date', key: 'date', width: 140, render: (d: string, r: any) => r.status === 'PUBLISHED' ? d : 'N/A' },
                  { title: 'Tác giả', dataIndex: 'author', key: 'author', width: 160, render: (_: any, r: any) => r.author || `Admin ${r.author_id}` },
