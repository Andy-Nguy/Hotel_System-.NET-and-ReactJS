...
const BASE_URLS = [
  ...
  "http://192.168.1.3:8080",    // Local network IP
...
function normalizeImageUrl(url: string | null | undefined, baseUrl: string = BASE_URLS[0]): string | undefined {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const processedData = (data || []).map((r: any) => {
          ...
          console.log(`🔄 Normalizing URL for ${r.tenPhong}: "${r.urlAnhPhong}" → "${normalizedUrl}"`);
...
export async function getRooms(): Promise<Room[]> {
  ...
  throw new Error("❌ Cannot connect to backend database. Please ensure backend is running and accessible.");
}
