...
const toAbsoluteUrl = (p?: string): string | undefined => {
  if (!p) return p;
...
const normalize = (d: any): BlogPost => {
  ...
  return {
    ...
    images: Array.isArray(images) ? images.map((i: string) => toAbsoluteUrl(i) || i) : [],
    date,
    excerpt,
    author,
    tags: typeof tags === "string" ? (tags ? tags.split(',').map((t: string) => t.trim()).filter(Boolean) : []) : (Array.isArray(tags) ? tags : []),
    content,
    externalLink,
    status,
    displayOrder,
...
export const getPublishedBlogs = async (): Promise<BlogPost[]> => {
  try {
    ...
    return arr
      ...
      .sort((a, b) => (typeof a.displayOrder === 'number' ? a.displayOrder : 999) - (typeof b.displayOrder === 'number' ? b.displayOrder : 999));
  } catch (e) {
    console.warn('getPublishedBlogs: API failed and no local fallback is configured', e);
...
export const getBlogBySlug = async (slugOrId: string | number): Promise<BlogPost> => {
  try {
    ...
    console.warn(`getBlogBySlug: API failed for ${slugOrId} and no local fallback is configured`, e);
