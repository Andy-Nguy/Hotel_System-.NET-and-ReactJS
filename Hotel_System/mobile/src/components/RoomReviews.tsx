...
const getAuthorName = (r: Review) => {
  ...
  const flag =
    r.IsAnonym ?? r.isAnonym ?? r.IsAnonymous ?? r.anonymous ?? r.isAnonymous;
  const isAnon = flag === true || flag === 1 || flag === "1" || flag === "true";
  if (isAnon) return "Ẩn danh";

  // Try nested customer object first
  const kh =
    r.khachHang || r.KhachHang || r.customer || r.user || r.KhachHangDTO;
  const nameFromKh =
    kh && (kh.HoTen || kh.hoTen || kh.fullName || kh.name || kh.ten);
...
const getContent = (r: Review) =>
  r.content || r.noiDung || r.NoiDung || r.body || "";
...
const RoomReviews: React.FC<Props> = ({ roomId, visible, onClose }) => {
  ...
  const loadPage = async (p: number) => {
    ...
    try {
      ...
      if (Array.isArray(res)) {
        ...
        items = res.result;
        totalCount =
          typeof res.total === "number" ? res.total : res.totalCount ?? null;
        ...
        totalCount =
          typeof res.total === "number" ? res.total : res.count ?? null;
      }
  ...
  const renderDistribution = () => {
    ...
    const totalCount =
      stats?.totalReviews ||
      dist.reduce((s: number, d: any) => s + (d.count || 0), 0) ||
      0;
    const rows = [5, 4, 3, 2, 1];
    return (
      <View style={{ marginBottom: 12 }}>
        {rows.map((st) => {
          const item = dist.find((d: any) => Number(d.stars) === st) || {
            count: 0,
          };
      ...
      <View style={styles.reviewCard}>
        ...
        <View style={styles.reviewFooter}>
          ...
          {content && content.length > 200 ? (
            <TouchableOpacity
              onPress={() => setExpandedMap((m) => ({ ...m, [id]: !m[id] }))}
            >
        ...
        <View style={styles.content}>
          ...
          {loading ? (
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <Text
                  style={{ textAlign: "center", marginTop: 24, color: "#666" }}
                >
                  Chưa có đánh giá
                </Text>
              }
