...
";
        await _emailService.SendEmailAsync(kh.Email, kh.HoTen ?? "Quý khách", subject, body);
    ...
    public async Task<ActionResult<HoaDonResponse>> CreateHoaDon([FromBody] HoaDonRequest request)
    {
        try
        {
            ...
            var hoaDon = new HoaDon
            {
                ...
                TienThanhToan = tongTienThanhToan,    // = TongTien - TienCoc
                // Khi tạo hóa đơn từ PaymentService nghĩa là giao dịch đã THÀNH CÔNG
                // -> Gán trạng thái thanh toán = 2 (Đã thanh toán)
            ...
            _logger.LogInformation($"✅ Đã tạo hóa đơn: {idHoaDon} - TrangThai={datPhong.TrangThai} (Chờ xác nhận), TrangThaiThanhToan=2, TienCoc={tienCocTuDB:N0}, TienThanhToan={tongTienThanhToan:N0}");
...
";
                    ...
                    await _emailService.SendEmailAsync(khachHang.Email, khachHang.HoTen ?? "Quý khách", emailSubject, emailBody);
